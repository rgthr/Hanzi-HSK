/* Hanzi 2.5.9 — persistent no-repeat scheduler for 10-question Extra Hard */
(()=>{
'use strict';
...
function pickTen(){
  const p=pool(); if(p.length<COUNT)return[];
  ...
  if(chosen.length<COUNT){
    ...
    chosen=[...chosen,...recycled.slice(0,COUNT-chosen.length)];
  }
...
let session=null;
function start(){const list=pickTen();if(list.length<COUNT)return;const dirs=shuffle(['zh-en','zh-en','zh-en','zh-en','zh-en','en-zh','en-zh','en-zh','en-zh','en-zh']);session={list:list.map((x,i)=>({...x,direction:dirs[i]})),score:0,missed:[]};render(0)}
function render(i){if(i>=session.list.length)return results();const item=session.list[i],z2e=item.direction==='zh-en',card=shell(`Hard ${i+1} of ${COUNT}`);card.innerHTML=`<div class="erEyebrow">Extra hard</div><div class="erTitle">Translate from memory</div><div class="erSub">No hints. Sentences do not repeat until the eligible pool has been exhausted.</div><div class="erDirection">${z2e?'CHINESE → ENGLISH':'ENGLISH → CHINESE'}</div><div class="erTranslatePrompt ${z2e?'zh':''}">${z2e?item.zh:item.en}</div><textarea class="erInput" autocomplete="off" autocorrect="off" spellcheck="false" placeholder="Type your translation"></textarea><button class="erSubmit">Check answer</button><div class="erCorrection"><b>Correct answer</b><span>${z2e?item.en:item.zh}</span><span class="erPy">${item.py}</span></div><button class="erNext" style="display:none">${i===COUNT-1?'See results':'Next'}</button>`;const input=card.querySelector('.erInput'),submit=card.querySelector('.erSubmit'),next=card.querySelector('.erNext');input.focus();submit.onclick=()=>{if(card.dataset.answered)return;card.dataset.answered='1';const ok=z2e?normEn(input.value)===normEn(item.en):normZh(input.value)===normZh(item.zh);if(ok)session.score++;else session.missed.push(item);input.style.borderColor=ok?'#22c55e':'#ec4899';card.querySelector('.erCorrection').classList.add('on');submit.style.display='none';next.style.display='block';const c=S()?.byH?.[item.h];if(c)S().mark?.(c,['recognition','meaning'],ok)};next.onclick=()=>render(i+1)}
function results(){const card=shell('Hard test complete');card.innerHTML=`<div class="erEyebrow">Extra hard complete</div><div class="erTitle">${session.score===COUNT?'Perfect translation':session.score>=8?'Excellent':'Keep building sentences'}</div><div class="erScore">${session.score}/${COUNT}</div><div class="erSub">Future Extra Hard tests will avoid these ${COUNT} sentences until the eligible pool has been cycled through.</div>${session.missed.length?`<div class="erMissed">${session.missed.map(x=>`<div class="erMiss">${x.zh}<br><span style="color:var(--green);font-weight:700">${x.py}</span><br><span style="color:var(--muted)">${x.en}</span></div>`).join('')}</div>`:''}<button class="erNext">Done</button>`;card.querySelector('.erNext').onclick=close}
function bind(){const b=document.getElementById('exploreHardBtn');if(!b)return;b.dataset.noRepeat='1';b.onclick=e=>{e.preventDefault();e.stopPropagation();start()};const n=pool().length;b.disabled=n<COUNT;b.textContent=n>=COUNT?`Extra hard · ${COUNT} translations`:`Extra hard · ${n}/${COUNT} sentences ready`}
